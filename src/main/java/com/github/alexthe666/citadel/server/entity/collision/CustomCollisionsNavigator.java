...
public class CustomCollisionsNavigator extends GroundPathNavigation {
    ...
    private boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3 p_179692_7_, double p_179692_8_, double p_179692_10_) {
        for (BlockPos blockpos : BlockPos.betweenClosed(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1))) {
            ...
            if (!(d0 * p_179692_8_ + d1 * p_179692_10_ < 0.0D) && !this.level.getBlockState(blockpos).isPathfindable(PathComputationType.LAND) || ((ICustomCollisions)mob).canPassThrough(blockpos, level.getBlockState(blockpos), null)) {
                return false;
            }
        }
            ...
            for (int k = i; k < i + sizeX; ++k) {
                for (int l = j; l < j + sizeZ; ++l) {
                    ...
                    if (!(d0 * p_179683_8_ + d1 * p_179683_10_ < 0.0D)) {
                        var pathnodetype = this.nodeEvaluator.getPathTypeOfMob(new PathfindingContext(this.level, this.mob), k, y - 1, l, this.mob);
                        mutable.set(k, y - 1, l);
